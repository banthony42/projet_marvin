...
/*    // Interrupt sur l'Uart receive
   ...
   // asm volatile ("nop");
    ...
    //marvin_set_periode(MARVIN_PR3, 15, TYPE_B, MARVIN_CONF_TIMER3, TIME_MSEC);
      ...
      marvin_attach_servo(&servo1, MARVIN_OC4, MARVIN_OC4RS, 550, 2300, OC_TIMER2, 20000);
...
// coder le watchdog timer si on reste bloquer dans une des fonctions
        ...
        {
            TMR1 = 0;
         //   marvin_move_servo(&servo2, test[i++]);
         //   marvin_move_servo(&servo1, test2[i2++]);
            if (i2 == 3)
                i2 = 0;
            if (i == 10)
                i = 0;
